...
export default function SignInPage() {
  ...
  const onSubmit: SubmitHandler<SignInValues> = async (values) => {
    ...
    try {
      ...
      console.error(error);
      const errorMessage = "Something went wrong while signing in. Please try again.";
      toast.error(errorMessage);
      setFormStatus({
        type: "error",
        message: errorMessage,
  ...
  const onInvalid = (errors: FieldErrors<SignInValues>) => {
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <motion.button
                ...
                className="absolute inset-y-0 right-0 flex items-center px-3 text-slate-400 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 rounded-r-xl"
            ...
            {errors.password && (
              <p id="password-error" className="mt-2 text-xs text-rose-200" role="alert">
