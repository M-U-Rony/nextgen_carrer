...
export default function JobDetailsPage() {
  ...
  const handleDownloadCVAsPDF = async () => {
    if (!customizedCV || !job) return;

    try {
      // Import PDF generator dynamically (client-side only)
      const { generatePDFFromText } = await import("@/lib/pdf-generator");
      const doc = generatePDFFromText(customizedCV);
      const fileName = `CV_${job.title.replace(/\s+/g, "_")}_${job.company.replace(/\s+/g, "_")}.pdf`;
      doc.save(fileName);
          ...
          >
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.button
                  onClick={handleDownloadCVAsPDF}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-2 text-blue-300 transition hover:bg-blue-500/20 hover:text-blue-200"
            ...
            <div className="mt-4 flex items-center justify-end gap-3">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-purple-500 px-4 py-2 text-sm font-semibold text-white shadow-lg transition hover:shadow-xl"
              >
